        ...
        st.markdown("""
            <div style="padding-top: 50px; padding-left: 20px;">
                <h1 style="font-size: 5rem; font-weight: 800; line-height: 1; margin-bottom: 1.5rem; color: white;">
                    Manage Your <br>
                    <span style="background: linear-gradient(90deg, #3b82f6, #a855f7); -webkit-background-clip: text; -webkit-text-fill-color: transparent;">Financial Future.</span>
                </h1>
                <p style="color: #94a3b8; font-size: 1.4rem; margin-bottom: 3rem; max-width: 85%; font-weight: 400; line-height: 1.6;">
                    Transform your data into a strategic advantage with AI-layered wealth management. An experience beyond standards awaits you.
                </p>
        ...
        with auth_container.container():
            ...
            if mode == "login":
                ...
                with st.form("login_form_v3"):
                    email = st.text_input("Corporate Email", placeholder="investor@fintrack.ai")
                    password = st.text_input("Secret Password", type="password", placeholder="••••••••")
                ...
                st.markdown("<p style='text-align: center; color: #64748b; font-size: 1rem; margin-bottom: 40px;'>Expand your limits today</p>", unsafe_allow_html=True)
                with st.form("register_form_v3"):
                    ...
                    if st.form_submit_button("Start Membership", use_container_width=True):
                        ...
                        elif len(password) < 6: st.error("❌ Password security is insufficient.")
                        else:
...
def main():
    if st.session_state.user:
        if not st.session_state.onboarding_complete:
            ...
            if choice == "🏛️ Dashboard":
            ...
            elif choice == "🧠 AI Analysis Center":
                render_ai_screen()
                ...
                st.header(choice)
                st.write("Module is under construction.")
    else:
        auth_screen()
