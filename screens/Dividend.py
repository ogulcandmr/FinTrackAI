...
def render_dividend_screen():
    st.markdown("""
        <div class="animate-page" style="background: rgba(15, 23, 42, 0.4); border-radius: 30px; padding: 40px; border: 1px solid rgba(255,255,255,0.05); margin-bottom: 2.5rem; backdrop-filter: blur(20px);">
            ...
            <p style="color: #94a3b8; font-size: 1.2rem; font-weight: 400;">Analyze historical data, explore sectoral yields, and simulate your retirement.</p>
        </div>
        """, unsafe_allow_html=True)

    tab1, tab2, tab3 = st.tabs(["🚀 Simulator (Compound Yield)", "🗓️ Upcoming Calendar", "📊 Sector Analysis"])

    with tab1:
        st.markdown("<h3 style='margin-bottom:20px; color: white;'>Simulator Screen (5-10-20 Years)</h3>", unsafe_allow_html=True)
        ...
        with col1:
            ...
            initial = st.number_input("Initial Capital (₺)", min_value=0, value=100000, step=10000)
            monthly = st.number_input("Monthly Additional Investment (₺)", min_value=0, value=5000, step=1000)
        ...
        with col2:
            ...
            fig.update_layout(plot_bgcolor='rgba(0,0,0,0)', paper_bgcolor='rgba(0,0,0,0)', font=dict(color='white'), margin=dict(l=0, r=0, t=30, b=0), xaxis=dict(showgrid=False, title="Year"), yaxis=dict(showgrid=True, gridcolor='rgba(255,255,255,0.1)', title="₺"))
            st.plotly_chart(fig, use_container_width=True)
            
            sc1, sc2, sc3 = st.columns(3)
            sc1.metric("Total Invested", f"₺ {total_inv:,.0f}")
            sc2.metric("Compound Interest Profit", f"₺ {(final_val - total_inv):,.0f}")
            sc3.metric("Final Portfolio Reached", f"₺ {final_val:,.0f}")
    ...
    with tab3:
        st.markdown("<h3 style='margin-bottom:20px; color: white;'>Sectoral Dividend Yield Tables</h3>", unsafe_allow_html=True)
        tickers = ["TUPRS.IS", "FROTO.IS", "DOAS.IS", "SISE.IS", "ENJSA.IS", "TTRAK.IS"]
        df_divs = fetch_dividend_history(tickers)
        
        col_a, col_b = st.columns([1.5, 1])
        with col_a:
            st.markdown("#### Company Based Data")
            st.dataframe(df_divs.style.format({"Last Dividend (₺)": "{:.2f} ₺", "Yield (%)": "%{:.2f}"}), use_container_width=True, hide_index=True)
