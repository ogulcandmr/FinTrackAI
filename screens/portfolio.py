...
def render_portfolio_screen():
    ...
    if not user:
        st.warning("You must be logged in to access the portfolio.")
        return
    ...
    st.markdown("""
        <div class="animate-page" style="background: rgba(15, 23, 42, 0.4); border-radius: 30px; padding: 40px; border: 1px solid rgba(255,255,255,0.05); margin-bottom: 2rem; backdrop-filter: blur(20px);">
            <h1 style="font-size: 3rem; margin-bottom: 0; font-weight: 800;">💼 Data Layer and Wallet Management</h1>
        </div>
    ...
    def _portfolio_content():
        ...
        with col1:
            st.markdown(card_css.format(
                title="📊 Total Value (₺)",
                value=f"{summary['total_value_tl']:,.2f} ₺",
                val_color="white",
                sub=f"{summary['total_value_usd']:,.2f} USD · Cost: {summary['total_cost_tl']:,.2f} ₺",
                sub_color="#94a3b8"
            ), unsafe_allow_html=True)
        with col2:
        ...
        with st.expander("➕ Add New Asset", expanded=not enriched_rows):
            # Bilindik hisse/kripto — hızlı ekle (satır hizalı)
            st.markdown("**Popular company stocks and cryptocurrencies**")
            st.markdown("""
                <style>
                ...
                if quick_submit and quick_choice and quick_qty > 0:
                    ...
                    if price and price > 0:
                        ...
                        if err:
                            ...
                            st.success(f"**{sym}** {quick_qty:,.4f} units added (current price: {price:,.2f} USD).")
                            st.rerun()
                    else:
                        st.error(f"Price could not be fetched for {sym}. Please 'Update PnL' and try again.")
            ...
            with r1:
                suggest_symbol = st.text_input("Stock or crypto symbol", key="suggest_sym", placeholder="e.g.: AAPL, TTWO, BTC, ETH, XRP")
            with r2:
                st.markdown("<br>", unsafe_allow_html=True)
            ...
            if get_price_btn and suggest_symbol:
                ...
                if p is not None and p > 0:
                    ...
                    st.success(f"**{suggest_symbol.strip().upper()}** current price: **{p:,.2f} USD** — Purchase price updated in the form below.")
                    st.rerun()
                else:
                    st.error("Price could not be fetched for this symbol. Please check and try again (e.g.: AAPL, TTWO, BTC, ETH).")
            ...
            with st.form("portfolio_add_form"):
                ...
                if st.form_submit_button("Add to Portfolio"):
                    if not (asset_id and price > 0 and quantity > 0):
                        st.error("Asset symbol, price (USD) and quantity are required (positive).")
                    else:
                        ...
                        if err:
                            ...
                            st.success("Position added. Prices are in USD; PnL is calculated in ₺ (1 USD = 44 ₺).")
                            st.rerun()

        # --- Hisseler: USD fiyat + TL kar/zarar ---
        st.markdown("### 📋 Assets")
        if st.button("🔄 Update PnL", key="manual_refresh_list", use_container_width=True):
            pass
        st.markdown("<br>", unsafe_allow_html=True)
        if not enriched_rows:
            st.info("No assets yet. You can add using 'Add New Asset' above.")
            return
                    ...
                    <div style="background: rgba(30, 41, 59, 0.4); border-radius: 16px; padding: 20px; margin-bottom: 12px; border: 1px solid rgba(255,255,255,0.06);">
                        <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 12px;">
                            ...
                            <div style="text-align: right;">
                                ...
                                <span style="margin-left: 14px; color: {pnl_color}; font-weight: 700;">{_fmt_money(pnl_tl)} ₺ ({_fmt_pct(row.get('pnl_pct', 0))})</span>
                            </div>
                        </div>
                    </div>
                    """, unsafe_allow_html=True)
                ...
                with col_b:
                    if st.button("🗑️ Delete", key=f"del_{rid}_{i}"):
                        ...
                        if ok:
                            ...
                            st.error(err or "Failed to delete.")
                st.markdown("---")
