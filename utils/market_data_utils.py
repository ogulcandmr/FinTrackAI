...
def _apply_sanity_filter(symbol: str, new_price_usd: Optional[float]) -> Optional[float]:
    ...
    if p < 0.000001 or p > 1_000_000_000:
        _LAST_PRICE_WARNING[sym] = "Price looked abnormal; last known good price was kept."
        return prev
    ...
    if p > prev * 5 or p < prev / 5:
        _LAST_PRICE_WARNING[sym] = "Unrealistic jump detected; last known good price was kept."
        return prev

    if rel > max_rel:
        _LAST_PRICE_WARNING[sym] = "Extreme price move detected; last known good price was kept."
        return prev
